...
CREATE OR REPLACE VIEW view_batimento_retorno AS
  ...
  FROM tb_remessa rem
    LEFT JOIN tb_retorno ret ON ret.remessa_id = rem.id_remessa
...
CREATE OR REPLACE VIEW view_titulo AS
  SELECT tit.id_titulo AS id_tituloremessa,
         ...
         arq.data_recebimento AS datarecebimento_arquivo_remessa,
         ...
         ins_apre.tipo_instituicao_id AS tipoinstituicao_instituicao,
    conf.id_confirmacao,
         ...
         arq_conf.data_recebimento AS datarecebimento_arquivo_confirmacao,
         ...
         arq_conf_bco.data_recebimento AS datarecebimento_arquivo_confirmacao_instituicao,
    ret.id_retorno,
         arq_ret.nome_arquivo AS nomearquivo_arquivo_retorno,
         arq_ret.id_arquivo AS id_arquivo_retorno,
         arq_ret.data_recebimento AS datarecebimento_arquivo_retorno,
         ...
         arq_ret_bco.data_recebimento AS datarecebimento_arquivo_retorno_instituicao,
    ret_can.id_titulo_retorno_cancelamento,
         ret_can.remessa_id AS id_remessa_cancelamento,
         CASE
         WHEN ret_can.id_titulo_retorno_cancelamento IS NOT NULL THEN ret_can.data_ocorrencia
    ...
    mun.id_municipio,
         ...
         WHEN ret_can.tipo_ocorrencia::text = 'A'::text THEN 'CANCELADO'::text
         ...
         WHEN ret.tipo_ocorrencia::text = 'A'::text THEN 'CANCELADO'::text
         ELSE NULL::text
  ...
  FROM tb_titulo tit
    ...
    LEFT JOIN tb_titulo_retorno_cancelamento ret_can ON ret_can.titulo_remessa_id = tit.id_titulo;
...
CREATE OR REPLACE VIEW view_remessa_pendente AS
  SELECT 'B'::text AS tipo_arquivo,
         ...
         mun.nome_municipio AS nomemunicipio_municipio,
         mun.cod_ibge AS cod_ibge_municipio, ins_ori.versao AS versao_instituicao
  FROM tb_titulo tit
    JOIN tb_remessa rem ON tit.remessa_id = rem.id_remessa
    JOIN tb_instituicao ins_ori ON rem.instituicao_origem_id = ins_ori.id_instituicao
    JOIN tb_instituicao ins_des ON rem.instituicao_destino_id = ins_des.id_instituicao
