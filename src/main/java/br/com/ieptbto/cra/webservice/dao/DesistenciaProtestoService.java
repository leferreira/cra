...
public class DesistenciaProtestoService extends CraWebService {
	...
	try {
	    ...
	    loggerCra.sucess(usuario, getTipoAcaoLog(), "O arquivo de Desistência de Protesto " + nomeArquivo
	...
	for (DesistenciaProtesto dp : arquivo.getRemessaDesistenciaProtesto().getDesistenciaProtesto()) {
		...
		titulo.setDataHora(DataUtil.localDateToStringddMMyyyy(new LocalDate())
	...
	try {
	    ...
	    e.printStackTrace();
	    loggerCra.error(getUsuario(), getTipoAcaoLog(), "Erro interno ao construir o arquivo " + nomeArquivo + "."
		    + e.getMessage(), e);
